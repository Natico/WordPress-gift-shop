...
function newstore_woocommerce_loop_columns() {
	...
	return get_theme_mod( 'themefarmer_woocommerce_loop_columns', 5 );
}
...
function newstore_wc_get_gallery_image_html( $attachment_id, $index, $main_image = false, $is_thumb = false) {
	$zoom_feature 	   = get_theme_mod( 'newstore_product_disable_zoom_feature', true );
...
function newstore_get_page_links_dropdown() {
	if (class_exists('WooCommerce')) {
		...
		// foreach ($links as $key => $link) {
		// 	// اگر $key برابر با "login" باشد، XXXX را جایگزین %1$s کن
		// 	// در غیر این صورت، اگر $key برابر با "register" باشد، yyyy را جایگزین %1$s کن
		// 	$replacement = ($key === "login") ? "login lrm-login" : (($key === "register") ? "register lrm-register" : esc_attr($key));
